...
from sofagent.agents import Agent
from sofagent.utils import is_correct, init_answer, read_json, read_prompts, get_avatar, get_color_style
...
class System(ABC):
    ...
    def __init__(self, task: str, config_path: str, leak: bool = False, web_demo: bool = False,
                 dataset: Optional[str] = None, *args, **kwargs) -> None:
        ...
        if 'supported_tasks' in self.config:
            assert isinstance(self.config['supported_tasks'], list) and self.task in self.config[
                'supported_tasks'], f'Task {self.task} is not supported by the system.'
        self.agent_kwargs = {
            'system': self,
        }
    ...
    def log(self, message: str, agent: Optional[Agent] = None, logging: bool = True, live_display: bool = True) -> None:
        """Log the message.
        ...
        Args:
            ...
            `live_display` (`bool`, optional): Whether to display logs live in Streamlit. Defaults to `True`.
        ...
        if self.web_demo:
            ...
            # Use an HTML span with the style from get_color_style
            color_css = get_color_style(role)
            # Ensure role is bold and message is properly appended.
            # Escape HTML special characters in `message` if it can contain them and they shouldn't be interpreted as HTML.
            # For now, assuming `message` is mostly plain text or already formatted markdown safe for this context.
            final_message = f'{get_avatar(role)}<span style="{color_css}"><strong>{role}</strong></span>: {message}'
            ...
            # Only display live if web_demo is true AND live_display is true
            if st.session_state.get('dev_mode', False):  # Check new flag
                st.markdown(f'{final_message}', unsafe_allow_html=True)  # Added unsafe_allow_html
